import { Chapter } from '../frontend/src/types/chapter';
import { LearningMaterial } from '../frontend/src/types/learningMaterials';
...
const mockLearningMaterials: Record<number, LearningMaterial[]> = {
  1: [
    {
      id: 1,
      chapter_id: 1,
...
export const testChapterRetrieval = async (): Promise<boolean> => {
  try {
...
export const testLearningMaterialsRetrieval = async (chapterId: number): Promise<boolean> => {
  try {
    // Simulate API call delay
    await new Promise(resolve => setTimeout(resolve, 100));
...
export const testReadingTimeEstimation = (chapter: Chapter): boolean => {
  try {
...
export const testContentPersonalization = (userId: number, chapterId: number, backgroundLevel: string): boolean => {
  try {
